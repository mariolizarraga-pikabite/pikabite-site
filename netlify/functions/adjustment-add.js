// netlify/functions/adjustment-add.js
...
export async function handler(event) {
  ...
  const delta = Number(body.delta || 0);  // can be + or -
  const reason = (body.reason || '').trim();

  if (!product || !delta || delta === 0) {
    return { statusCode: 400, headers: cors(origin), body: 'Missing/invalid product/delta' };
  }
  ...
  const { error } = await supabase.from('stock_ledger').insert({
    product, delta, source: 'adjustment', meta: { reason }
  });
